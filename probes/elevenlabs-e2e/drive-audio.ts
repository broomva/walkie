...
async function speak(text: string): Promise<Buffer> {
  ...
  const ff = Bun.spawnSync(["ffmpeg", "-y", "-loglevel", "error", "-i", aiff,
...
const QUEUE = [{
  ticket: "t-8831", workspace: "seaslug", state: "Needs you",
...
function postToSession(text: string): Promise<boolean> {
  return new Promise((resolve) => {
    ...
    sock.on("connect", () => {
      ...
      setTimeout(() => { sock.end(); resolve(true); }, 400);
    });
...
const ws = new WebSocket(`wss://api.elevenlabs.io/v1/convai/conversation?agent_id=${AGENT}`,
  { headers: { "xi-api-key": KEY } } as any);
const send = (o: unknown) => ws.send(JSON.stringify(o));

const seen: string[] = [];
const transcripts: string[] = [];
let audioBytesOut = 0, audioEvents = 0;
let drained = false, sawAnswer = false, spokeAnswer = false;
...
async function stream(pcm: Buffer, label: string, tailMs = 2000) {
  ...
  console.log(`    [mic] ${label} — ${(pcm.length / 32000).toFixed(1)}s speech + ${tailMs}ms silence`);
...
const finish = async (code: number) => {
  ...
  check("the Claude Code session woke from a spoken answer", found,
...
const timer = setTimeout(() => { check("completed within 180s", false, "timed out"); finish(1); }, 180_000);
...
ws.addEventListener("message", async (m: any) => {
  ...
  if (ev.type === "conversation_initiation_metadata") {
    check("conversation started in audio mode", true,
  ...
  if (ev.type === "client_tool_call") {
    ...
    if (c.tool_name === "answer_ask") {
      ...
      check("spoken answer reached the tool with the right ticket", p.ticket === "t-8831",
        `ticket=${p.ticket}`);
      check("the spoken answer survived speech to text to tool",
        /genesis/i.test(String(p.answer ?? "")), String(p.answer ?? "").slice(0, 60));
      const posted = await postToSession(
        `walkie audio e2e: ticket ${p.ticket} answered "${p.answer}". ` +
  ...
  if (ev.type === "agent_response") {
    ...
    if (drained && !spokeAnswer && !sawAnswer) {
      spokeAnswer = true;
      check("agent spoke the queued ask aloud", /seaslug|attach|genesis/i.test(t),
        t.slice(0, 80).replace(/\n/g, " "));
      await stream(await speak("Go with Genesis sessions only, the safest one."),
                   "go with genesis sessions only");
    } else if (sawAnswer) {
      clearTimeout(timer);
      // Assertions that need the whole run in hand.
      const heard = transcripts.join(" ").toLowerCase();
      check("ASR heard the first utterance", /what needs me/.test(heard),
        transcripts[0] ?? "(none)");
      check("ASR heard the workspace answer", /genesis/.test(heard),
        transcripts.find((x) => /genesis/i.test(x)) ?? "(none)");
      check("agent audio actually came back", audioEvents > 0 && audioBytesOut > 20000,
